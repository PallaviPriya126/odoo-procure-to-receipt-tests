...
test('TC01 create RFQ shows status RFQ and correct line total', async ({ authenticatedPage: page }) => {
...
test('TC08 saving an RFQ without a vendor shows an error', async ({ authenticatedPage: page }) => {
  const seed = JSON.parse(await readFile('.auth/seed.json', 'utf8'));
